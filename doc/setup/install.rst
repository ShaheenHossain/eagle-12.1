...
===============
Installing Eagle ERP
===============

There are multiple ways to install Eagle ERP, or not install it at all, depending
on the intended use case.

This documents attempts to describe most of the installation options.

:ref:`setup/install/online`
    The easiest way to use Eagle ERP in production or to try it.

:ref:`setup/install/packaged`
    Suitable for testing Eagle ERP, developing modules and can be used for
    long-term production use with additional deployment and maintenance work.

:ref:`setup/install/source`
    Provides greater flexibility:  e.g. allow multiple running Eagle ERP versions on
...
There are two different Editions_ of Eagle ERP: the Community and Enterprise versions.
...
To simply get a quick idea of Eagle ERP, demo_ instances are available. They are
...
Trivial to start with, fully managed and migrated by Eagle ERP S.A., Eagle ERP's SaaS_
provides private instances and starts out free. It can be used to discover and
test Eagle ERP and do non-code customizations (i.e. incompatible with custom modules
or the Eagle ERP Apps Store) without having to install it locally.

Can be used for both testing Eagle ERP and long-term production use.
...
Eagle ERP provides packaged installers for Windows, deb-based distributions
...
Eagle ERP will automatically be started at the end of the installation.
...
Eagle ERP 12.0 'deb' package currently supports `Debian Stretch`_, `Ubuntu 18.04`_ or above.

Prepare
^^^^^^^

Eagle ERP needs a `PostgreSQL`_ server to run properly. The default configuration for
the Eagle ERP 'deb' package is to use the PostgreSQL server on the same host as your
Eagle ERP instance. Execute the following command as root in order to install
PostgreSQL server :
...
Eagle ERP S.A. provides a repository that can be used with  Debian and Ubuntu
distributions. It can be used to install Eagle ERP Community Edition by executing the
...
This will install Eagle ERP as a service, create the necessary PostgreSQL_ user
...
.. warning:: Debian 9 and Ubuntu do not provide a package for the python module
             num2words.
             Textual amounts will not be rendered by Eagle ERP and this could cause
...
Eagle ERP 12.0 'rpm' package supports Fedora 26.
As of 2017, CentOS does not have the minimum Python requirements (3.5) for
Eagle ERP 12.0.

Prepare
^^^^^^^
Eagle ERP needs a `PostgreSQL`_ server to run properly. Assuming that the 'sudo'
...
Eagle ERP S.A. provides a repository that can be used with the Fedora distibutions.
It can be used to install Eagle ERP Community Edition by executing the following
commands:
...
The source "installation" really is about not installing Eagle ERP, and running
it directly from source instead.

This can be more convenient for module developers as the Eagle ERP source is
...
It also makes starting and stopping Eagle ERP more flexible and explicit than the
...
easily keeping (and running) multiple versions of Eagle ERP side-by-side.
...
* Python 3.5+.
  ...
  - on Windows, use `the official Python 3 installer
    ...
    .. note:: if Python is already installed, make sure it is 3.5 or above,
              previous versions are not compatible with Eagle ERP.

* PostgreSQL, to use a local database

  After installation you will need to create a postgres user: by default the
  only user is ``postgres``, and Eagle ERP forbids connecting as ``postgres``.
  ...
  - on Windows, use `PostgreSQL for windows`_ then
    ...
    - create a postgres user with a password using the pg admin gui: open
      ...
      The user and password must be passed to Eagle ERP using either the
...
* Python dependencies listed in the :file:`requirements.txt` file.
  ...
  - on Windows you need to install some of the dependencies manually, tweak the
    ...
    command from a cmd.exe prompt (replace ``\YourEagle ERPPath`` by the actual
    path where you downloaded Eagle ERP):

    .. code-block:: doscon

        C:\> cd \YourEagle ERPPath
        C:\YourEagle ERPPath> C:\Python35\Scripts\pip.exe install -r requirements.txt
...
There are two ways to obtain the Eagle ERP source code: zip or git.

* Eagle ERP zip can be downloaded from  our nightly_ server or our Download_  page,
...
* git allows simpler update and easier switching between different versions
  of Eagle ERP. It also simplifies maintaining non-module patches and
  contributions.  The primary drawback of git is that it is significantly
  larger than a tarball as it contains the entire history of the Eagle ERP project.
...
.. note:: The Enterprise git repository **does not contain the full Eagle ERP
    ...
    Eagle ERP installation

Running Eagle ERP
------------

Once all dependencies are set up, Eagle ERP can be launched by running ``eagle-bin``.
...
* PostgreSQL host, port, user and password.
  ...
  Eagle ERP has no defaults beyond
...
.. code-block:: doscon
    ...
    C:\YourEagle ERPPath> python3 eagle-bin -w eagle -r eagle --addons-path=addons,../mymodules --db-filter=mydb$
...
This section will explain how to run Eagle ERP in a such isolated Python environment.
...
This section assumes that you obtained the Eagle ERP sources from the zip file or the
...
Now, let's install the tools required to build Eagle ERP dependencies if needed:
...
Now we can create a virtual environment for Eagle ERP like this:
...
Now let's install the Eagle ERP required python packages:
...
The full documentation on how to use Eagle ERP with Docker can be found on the
official Eagle ERP `docker image <https://registry.hub.docker.com/_/eagle/>`_ page.
