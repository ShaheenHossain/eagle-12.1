...
.. warning::
    ...
    * This guide assumes :ref:`an installed Eagle ERP <setup/install>`

Creating a basic module
=======================

In Eagle ERP, tasks are performed by creating modules.

Modules customize the behavior of an Eagle ERP installation, either by adding new
behaviors or by altering existing ones (including behaviors added by other
modules).

:ref:`Eagle ERP's scaffolding <reference/cmdline/scaffold>` can setup a basic
...
display logic. Eagle ERP allows any Python templating system, but provides its
...
Finally restart Eagle ERP and update the module's data (to install the template)
by going to :menuselection:`Settings --> Modules --> Modules -->
Academy` and clicking :guilabel:`Upgrade`.

.. tip::

    Alternatively, Eagle ERP can be restarted :option:`and update modules at
...
Storing data in Eagle ERP
====================

:ref:`Eagle ERP models <reference/orm/model>` map to database tables.
...
Eagle ERP bundles a module dedicated to building websites.

So far we've used controllers fairly directly, but Eagle ERP 8 added deeper
...
You're now in Eagle ERP "proper": the administrative interface. For now click on
...
restart Eagle ERP, access http://localhost:8069/academy/Alice/ and
...
Restart Eagle ERP, access http://localhost:8069/academy/2, note how the old value
...
Eagle ERP provides an additional converter called ``model`` which provides records
...
Restart Eagle ERP and upgrade the module, then you can visit each teacher's page.
...
Restart Eagle ERP and update the views, reload the teacher's page and… the field
...
Restart Eagle ERP and upgrade the module, there is now a placeholder under the
...
A brief and incomplete introduction to the Eagle ERP administration
--------------------------------------------------------------

The Eagle ERP administration was briefly seen during the `website support`_ section.
We can go back to it using :menuselection:`Administrator --> Administrator` in
the menu (or :guilabel:`Sign In` if you're signed out).

The conceptual structure of the Eagle ERP backend is simple:

#. first are menus, a tree (menus can have sub-menus) of records. Menus
   without children map to…
#. actions. Actions have various types: links, reports, code which Eagle ERP should
   execute or data display. Data display actions are called *window actions*,
   and tell Eagle ERP to display a given *model* according to a set of views…
...
Editing in the Eagle ERP administration
----------------------------------

By default, an Eagle ERP model is essentially invisible to a user. To make it
...
:ref:`view <reference/views>`) Eagle ERP will automatically create a basic one
...
Eagle ERP provides technical models, which don't directly fulfill business needs
but which add capabilities to business objects without having to build
them by hand.

One of these is the *Chatter* system, part of Eagle ERP's email and messaging
...
Eagle ERP also provides business models which allow using or opting in business
needs more directly. For instance the ``website_sale`` module sets up an
e-commerce site based on the products in the Eagle ERP system. We can easily make
...
restart Eagle ERP, update your module, there is now a :guilabel:`Shop` section in
