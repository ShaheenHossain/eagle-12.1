...
.. note::
    ...
    Python 2 and Python 3, use the :data:`eagle.tools.pycompat.PY2` flag.
...
.. important::
    ...
    * ``(int, long)`` for type-checking purposes must be replaced by
      :py:data:`eagle.tools.pycompat.integer_types`
...
For easier type-testing, :mod:`eagle.tools.pycompat` provides the following
constants:

* :data:`~eagle.tools.pycompat.string_types` is an alias/type tuple for testing
  string types, essentially a replacement of testing for ``basestring`` or
  ``(str, unicode)``.
* :data:`~eagle.tools.pycompat.text_type` is the proper *text* type for the
