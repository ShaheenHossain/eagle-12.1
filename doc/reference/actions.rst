...
Creates a new record, from scratch (via :meth:`~eagle.models.Model.create`)
or by copying an existing record (via :meth:`~eagle.models.Model.copy`)
...
``crud_model_id``
    ...
    ``new_other``
``ref_object``
    :class:`~eagle.fields.Reference` to an arbitrary record to copy, used if
